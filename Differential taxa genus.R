...
d$samples <- d$samples %>%
  separate(group, into = c("treatment", "time", "age"), sep = "\\.", remove = FALSE)%>%
  mutate(
    treatment = as.factor(treatment),
    time = as.factor(time),
    age = as.factor(age))%>%
  mutate(treatment_time = as.factor(paste(treatment, time, sep = "_")))


plotMDS(d, top = 1000, col = as.numeric(d$samples$group),
        main = "PCoA of genera between groups")
par(xpd =T)
legend(2.8,-1,col = as.numeric(d$samples$group), pch=c(15), legend=c("Exercise week 0","Exercise week 6",
...
plotMD(qlf)
# Filter for features with significant adj p-value (FDR < 0.05)
res <- as.data.frame(topTags(qlf, n=7, sort.by='p.value'))%>%
  filter(FDR < 0.05)
  text(res$logCPM, res$logFC, labels = rownames(res), col = "black", cex = 0.5,
       pos = 3)
is.de <- decideTestsDGE(qlf)
...
plotMD(qlf)
# Filter for features with significant adj p-value (FDR < 0.05)
res <- as.data.frame(topTags(qlf, n=10))%>%
  filter(FDR < 0.05)
  text(res$logCPM, res$logFC, labels = rownames(res), col = "black", cex = 0.5,
       pos = 3)
is.de <- decideTestsDGE(qlf)
...
out <- by(data = all.comp, INDICES = all.comp$comparison, FUN = function(m) {
  m <- droplevels(m)
  m <- ggplot(m, aes(x = group, y = value, fill  = tax)) + 
    geom_bar(position = "dodge",stat = "identity") +
    theme(legend.key.size = unit(0.1, 'cm'), 
          legend.text = element_text(size = 5),
          legend.title = element_text(size = 7),
          legend.position.inside = 1,1
    )
})
...
pcoa <- DGEList(counts = df5, group = Group3)
keep <- filterByExpr(pcoa, group=Group3)
pcoa <- pcoa[keep, , keep.lib.sizes=FALSE]
pcoa <- estimateDisp(pcoa)
pcoa <- calcNormFactors(pcoa)

  
plotMDS(pcoa, top = 1000, col = as.numeric(pcoa$samples$group),
          main = "PCoA of genera for young vs old mice",pch=16)
legend("topleft", col = c("black", "salmon"), pch=c(15), legend=c("Old mice","Young mice"))
...
plotMD(qlf)
# Filter for features with significant adj p-value (FDR < 0.05)
res <- as.data.frame(topTags(qlf, n=53))%>%
  filter(FDR < 0.05)
text(res$logCPM, res$logFC, labels = rownames(res), col = "black", cex = 0.5,
     pos = 3)
is.de <- decideTestsDGE(qlf)
...
summary(is.de)
# Filter for features with significant adj p-value (FDR < 0.05)
res <- as.data.frame(topTags(qlf, n=23))%>%
  filter(FDR < 0.05)
text(res$logCPM, res$logFC, labels = rownames(res), col = "black", cex = 0.5,
     pos = 3)
is.de <- decideTestsDGE(qlf)
...
rel.abund.table <- cpm
rel.abund.table$genus <- rownames(rel.abund.table)
...
rel.abund.table <- rel.abund.table%>%
  pivot_longer(cols = -genus, names_to = 'SampleID', values_to = 'cpm')
...
ggplot(data = x,aes(x = Treatment_time, y = abundance, fill = genus), show.legend = F) + 
  geom_bar(stat = "identity") +
  theme_minimal() + 
  scale_fill_discrete(breaks=y$genus)+
