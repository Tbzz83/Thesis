...
df3 <- read_csv('wf-metagenomics-counts-species.csv')%>%
  select(-MEY15_cntrl)%>%
  select(-c(total, superkingdom, kingdom, phylum, class,
            order, genus,family, tax))%>%
...
plotMDS(d, top = 1000, col = as.numeric(d$samples$group), pch = 16,
        main = "PCoA of species between groups")
legend("topleft", col = as.numeric(d$samples$group), pch=c(15), legend=c("Group A","Group B"))
...
# Filter for features with significant adj p-value (FDR < 0.05)
res <- as.data.frame(topTags(qlf, sort.by = "p.value", n=9))%>%
  filter(FDR < 0.05)
text(res$logCPM, res$logFC, labels = rownames(res), col = "black", cex = 0.5,
     pos = 3)
is.de <- decideTestsDGE(qlf)
...
plotMD(qlf)
# Filter for features with significant adj p-value (FDR < 0.05)
res <- as.data.frame(topTags(qlf, n=5))%>%
  filter(FDR < 0.05)
  text(res$logCPM, res$logFC, labels = rownames(res), col = "black", cex = 0.5,
       pos = 3)
is.de <- decideTestsDGE(qlf)
...
# ----------------------------------------------
# Comparing the change in ex old to ex young and control old to control young
...
rel.abund.table <- cpm
rel.abund.table$species <- rownames(rel.abund.table)
...
rel.abund.table <- rel.abund.table%>%
  pivot_longer(cols = -species, names_to = 'SampleID', values_to = 'cpm')
...
x <- rel.abund.table %>%
  group_by(Treatment_time, species)%>%
...
y <- x %>%
  ...
  slice_head(n = 15) 
legend_keep <- y$species

ggplot(data = x,aes(x = Treatment_time, y = abundance, fill = species), show.legend = F) + 
  geom_bar(stat = "identity") +
  theme_minimal() + 
  scale_fill_discrete(breaks=y$species) +
